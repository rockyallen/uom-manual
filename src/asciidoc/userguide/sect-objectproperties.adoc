...
The 'might' is a fairly strong indication that the method should not be used, although in fact the example Quantitys are considered equal in {product}.

Equivalence is the normal test to use: it converts to common units where possible, then compares the amounts and the dimensions of the quantities.
This lets it work across different units of the same Quantity type, and even across systems of units.

Caveats:.footnote[I am at my Java limits here, and making it up]
...
Quantitys are not declared as comparable, but you can use them in Sorted Collections anyway:.footnote:[Quantity doesn't implement Comparable (Why not?) but get Quantity does, and all creation methods seem to delegate to it]
...
Yes, with no restrictions (check).

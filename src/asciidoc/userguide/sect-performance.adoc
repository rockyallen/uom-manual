...
Instances with identical constructor arguments are immutable and functionally identical, so you can define Units and frequently used constants just once:
...
[source,java]
----
method(Quantity<Mass> q_input) {
double q_internal = q_input.toSystemUnits().getValue().doubleValue(); 
...
}
----
...
Each instance of the +ThermoCoupleReading+ class below serialises as 2 floats, but keeps the type-safety of Quantity.

[source,java]
----
include::{basedir}/src/main/java/ThermoCoupleReading.java[tag="class"]
----
